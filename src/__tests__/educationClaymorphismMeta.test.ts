...
const ids = ['education-claymorphism-light', 'education-claymorphism-dark'] as const;
...
describe('Educational Claymorphism samples metadata', () => {
  it('深浅色目录都存在且包含完整文件', () => {
    for (const id of ids) {
  ...
  it('meta 字段符合教育类 claymorphism 规范', () => {
    for (const id of ids) {
      const meta = readMeta(id);
      expect(meta.id).toBe(id);
      expect(meta.entry).toBe('index.html');
      expect(meta.industry).toBe('education');
      expect(meta.density).toBe('comfortable');
      expect(meta.style).toContain('Claymorphism');
      expect(meta.style).toContain('Education');
